      ...
      <section className="py-24 px-6 min-h-[80vh] flex items-center">
        <div className="mx-auto w-full max-w-md">
          <div className="text-center mb-10">
            <span className="seal text-base mx-auto mb-6">黃</span>
            <h1 className="font-serif text-4xl mb-2">{mode === "signin" ? "Chào mừng trở lại" : "Tham gia cùng chúng tôi"}</h1>
            <p className="text-muted-foreground text-sm">{mode === "signin" ? "Đăng nhập để tiếp tục hành trình tu dưỡng" : "Tạo tài khoản miễn phí để bắt đầu"}</p>
          </div>
          <div className="ink-card rounded-sm p-8">
            <Button type="button" variant="outline" disabled={loading} onClick={handleGoogle} className="w-full rounded-sm mb-4">
            ...
            <p className="text-center text-sm text-muted-foreground mt-6">
              ...
              <button onClick={() => setMode(mode === "signin" ? "signup" : "signin")} className="text-imperial hover:underline font-medium">
                {mode === "signin" ? "Đăng ký ngay" : "Đăng nhập"}
              </button>
            </p>
          </div>
          <p className="text-center text-xs text-muted-foreground mt-6">
            Khi đăng ký, bạn đồng ý với <a href="#" className="underline">Điều khoản</a> và <a href="#" className="underline">Chính sách bảo mật</a> của chúng tôi.
          </p>
        </div>
      </section>
    </SiteLayout>
